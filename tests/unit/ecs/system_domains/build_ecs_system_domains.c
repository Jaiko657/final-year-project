...
int main(int argc, char **argv)
{
    ...
    if (!nob_mkdir_if_not_exists("build/tests/obj/ecs_system_domains")) return 1;
    if (!nob_mkdir_if_not_exists("build/tests/plugins")) return 1;

    Nob_File_Paths test_sources = {0};
    nob_da_append(&test_sources, "tests/unit/ecs/system_domains/test_ecs_system_domains.c");

    const char *runner_path = "build/tests/gen/tests_ecs_system_domains_runner.c";
    if (!generate_unity_runner("ecs_system_domains", &test_sources, runner_path)) return 1;
    ...
    const char *includes =
        ...
        "-I src "
        ""
        "-I tests/unit/ecs/system_domains "
    ...
    nob_da_append(&sources, "src/modules/ecs/ecs_input_system.c");
    nob_da_append(&sources, "src/modules/ecs/ecs_movement_system.c");
    nob_da_append(&sources, "src/modules/ecs/ecs_physics_system.c");
    nob_da_append(&sources, "tests/unit/ecs/system_domains/ecs_system_domains_stubs.c");
    nob_da_append(&sources, "tests/unit/ecs/system_domains/test_ecs_system_domains.c");
    ...
    for (size_t i = 0; i < sources.count; ++i) {
        ...
        const char *obj = nob_temp_sprintf("build/tests/obj/ecs_system_domains/%s.o", stem);
    ...
    Nob_Cmd cmd = {0};
    {
        ...
        nob_sb_append_cstr(&link, "-o build/tests/plugins/tests_ecs_system_domains.so -lm");
