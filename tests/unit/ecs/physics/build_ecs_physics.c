...
int main(int argc, char **argv)
{
    ...
    if (!nob_mkdir_if_not_exists("build/tests/obj/ecs_physics")) return 1;
    if (!nob_mkdir_if_not_exists("build/tests/plugins")) return 1;

    Nob_File_Paths test_sources = {0};
    nob_da_append(&test_sources, "tests/unit/ecs/physics/test_ecs_physics.c");

    const char *runner_path = "build/tests/gen/tests_ecs_physics_runner.c";
    if (!generate_unity_runner("ecs_physics", &test_sources, runner_path)) return 1;
    ...
    nob_da_append(&sources, "tests/unit/ecs/physics/ecs_physics_stubs.c");
    nob_da_append(&sources, "tests/unit/ecs/physics/test_ecs_physics.c");
    ...
    for (size_t i = 0; i < sources.count; ++i) {
        ...
        const char *obj = nob_temp_sprintf("build/tests/obj/ecs_physics/%s.o", stem);
    ...
    Nob_Cmd cmd = {0};
    {
        ...
        nob_sb_append_cstr(&link, "-o build/tests/plugins/tests_ecs_physics.so -lm");
