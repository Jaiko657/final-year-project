...
int main(int argc, char **argv)
{
    ...
    if (!nob_mkdir_if_not_exists("build/tests/obj/ecs_prefab_loading")) return 1;
    if (!nob_mkdir_if_not_exists("build/tests/plugins")) return 1;

    Nob_File_Paths test_sources = {0};
    nob_da_append(&test_sources, "tests/unit/ecs/prefab_loading/test_ecs_prefab_loading.c");

    const char *runner_path = "build/tests/gen/tests_ecs_prefab_loading_runner.c";
    if (!generate_unity_runner("ecs_prefab_loading", &test_sources, runner_path)) return 1;
    ...
    const char *includes =
        ...
        "-I src "
        ""
        "-I tests/unit/ecs/prefab_loading "
    ...
    for (size_t i = 0; i < sources.count; ++i) {
        ...
        const char *obj = nob_temp_sprintf("build/tests/obj/ecs_prefab_loading/%s.o", stem);
    ...
    Nob_Cmd cmd = {0};
    {
        ...
        nob_sb_append_cstr(&link, "-o build/tests/plugins/tests_ecs_prefab_loading.so -lm");
