...
void test_ecs_systems_run_phase_orders_by_order_field(void)
{
    ...
    ecs_register_system(PHASE_SIM_PRE, 200, sys_b, "b");
    ecs_register_system(PHASE_SIM_PRE, 100, sys_a, "a");
    ecs_register_system(PHASE_SIM_PRE, 300, sys_c, "c");

    ecs_run_phase(PHASE_SIM_PRE, 0.0f, NULL);
...
void test_ecs_systems_get_phase_systems_returns_sorted_list(void)
{
    ...
    ecs_register_system(PHASE_PHYSICS, 50, sys_b, "b");
    ecs_register_system(PHASE_PHYSICS, 10, sys_a, "a");

    const ecs_system_info_t* list = NULL;
    size_t n = ecs_get_phase_systems(PHASE_PHYSICS, &list);
...
void test_ecs_systems_caps_system_count(void)
{
    ...
    // Default cap is 64 in ecs_systems.c.
    for (int i = 0; i < 100; ++i) {
        ecs_register_system(PHASE_INPUT, i, sys_a, "x");
    }

    const ecs_system_info_t* list = NULL;
    size_t n = ecs_get_phase_systems(PHASE_INPUT, &list);
