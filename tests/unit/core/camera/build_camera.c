...
int main(int argc, char **argv)
{
    ...
    if (!nob_mkdir_if_not_exists("build/tests/obj/camera")) return 1;
    if (!nob_mkdir_if_not_exists("build/tests/plugins")) return 1;

    Nob_File_Paths test_sources = {0};
    nob_da_append(&test_sources, "tests/unit/core/camera/test_camera.c");

    const char *runner_path = "build/tests/gen/tests_camera_runner.c";
    if (!generate_unity_runner("camera", &test_sources, runner_path)) return 1;
    ...
    nob_da_append(&sources, "src/modules/core/camera.c");
    ...
    for (size_t i = 0; i < sources.count; ++i) {
        ...
        const char *obj = nob_temp_sprintf("build/tests/obj/camera/%s.o", stem);
    ...
    Nob_Cmd cmd = {0};
    {
        ...
        nob_sb_append_cstr(&link, "-o build/tests/plugins/tests_camera.so -lm");
