...
int main(int argc, char **argv)
{
    ...
    if (!nob_mkdir_if_not_exists("build/tests/obj/dynarray")) return 1;
    if (!nob_mkdir_if_not_exists("build/tests/plugins")) return 1;

    Nob_File_Paths test_sources = {0};
    nob_da_append(&test_sources, "tests/unit/core/dynarray/test_dynarray.c");

    const char *runner_path = "build/tests/gen/tests_dynarray_runner.c";
    if (!generate_unity_runner("dynarray", &test_sources, runner_path)) return 1;
    ...
    nob_da_append(&sources, "tests/unit/core/dynarray/test_dynarray.c");
    ...
    for (size_t i = 0; i < sources.count; ++i) {
        ...
        const char *obj = nob_temp_sprintf("build/tests/obj/dynarray/%s.o", stem);
    ...
    Nob_Cmd cmd = {0};
    {
        ...
        nob_sb_append_cstr(&link, "-o build/tests/plugins/tests_dynarray.so -lm");
