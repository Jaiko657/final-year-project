...
int main(int argc, char **argv)
{
    ...
    if (!nob_mkdir_if_not_exists("build/tests/obj/prefab")) return 1;
    if (!nob_mkdir_if_not_exists("build/tests/plugins")) return 1;

    Nob_File_Paths test_sources = {0};
    nob_da_append(&test_sources, "tests/unit/prefab/test_prefab.c");
    nob_da_append(&test_sources, "tests/unit/prefab/test_prefab_cmp_common.c");
    nob_da_append(&test_sources, "tests/unit/prefab/test_prefab_cmp_pos.c");
    nob_da_append(&test_sources, "tests/unit/prefab/test_prefab_cmp_vel.c");
    nob_da_append(&test_sources, "tests/unit/prefab/test_prefab_cmp_col.c");
    ...
    if (!generate_unity_runner("prefab", &test_sources, runner_path)) return 1;
    ...
    nob_da_append(&sources, "tests/unit/prefab/test_prefab_cmp_pos.c");
    nob_da_append(&sources, "tests/unit/prefab/test_prefab_cmp_vel.c");
    nob_da_append(&sources, "tests/unit/prefab/test_prefab_cmp_col.c");
    ...
    for (size_t i = 0; i < sources.count; ++i) {
        ...
        const char *obj = nob_temp_sprintf("build/tests/obj/prefab/%s.o", stem);
    ...
    Nob_Cmd cmd = {0};
    {
        ...
        nob_sb_append_cstr(&link, "-o build/tests/plugins/tests_prefab.so -lm");
