#include "unity.h"

#include "modules/prefab/prefab_cmp.h"

static prefab_component_t make_comp(const char* type_name, prefab_kv_t* props, size_t prop_count)
{
    return (prefab_component_t){
        .id = ENUM_COMPONENT_COUNT,
        .type_name = (char*)type_name,
        .props = props,
        .prop_count = prop_count,
        .anim = NULL,
        .override_after_spawn = false,
    };
}

void test_prefab_cmp_item_build_parses_kind(void)
{
    prefab_kv_t props[] = {
        { .name = "kind", .value = "hat" },
    };
    prefab_component_t comp = make_comp("ITEM", props, 1);

    prefab_cmp_item_t out = {0};
    TEST_ASSERT_TRUE(prefab_cmp_item_build(&comp, NULL, &out));
    TEST_ASSERT_EQUAL_INT(ITEM_HAT, (int)out.kind);
}

